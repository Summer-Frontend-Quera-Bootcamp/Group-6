...
export async function fetchBoards(
  ...
  try {
    const BoardsResponse = await AXIOS.get(
      `/workspaces/${space}/projects/${project}/boards/`
    );
    const thisProject = await AXIOS.get(
      `/workspaces/${space}/projects/${project}/`
    );
    ...
    const boardData: IBoardData[] = await Promise.all(boardsPromises);
